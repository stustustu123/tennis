...
class MySQLSetup(object):
	...
	def build_table_game(self):
		...
		with self.con:
			...
			cur.execute("CREATE TABLE Games(Id INT PRIMARY KEY AUTO_INCREMENT, playerA BOOL, playerB BOOL, game_time TIME) ENGINE=INNODB")
	...
	def print_edit(self, table):
		with self.con: 
			...
			for i in range(cur.rowcount):
				...
				print ('Database contains: %s' % (str(row[0:a])))
